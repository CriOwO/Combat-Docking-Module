...
public class combat_docking_AI extends BaseShipAI {
    ...
    float CRmarker = 0.4f;
    float CRrestored = 0.20f;
    float CRmax = 1f;
    float refit_timer = 0f;
    String timer = "0";
    ...
    public void advance(float amount) {
    	...
    	if (ship == playerShip) 
			Global.getCombatEngine().maintainStatusForPlayerShip("AceSystem3", "graphics/ui/icons/icon_repair_refit.png", "LANDING AUTOPILOT" , "INITIALIZED" ,false);
    	...
    	if(ship.isFinishedLanding()) {
    		...
    		refit_timer = Math.round((BASE_REFIT.getElapsed()/BASE_REFIT.getMaxInterval()) * 100f); //Gives timer as a percentage rounded up.
    		
    		timer = String.valueOf(refit_timer);
    		
    		if (ship == playerShip && BASE_REFIT.getElapsed() >= 0f) //Dislay count down timer
				Global.getCombatEngine().maintainStatusForPlayerShip("AceSystem", "graphics/ui/icons/icon_repair_refit.png","REFITTING . . .", timer+"%" ,false);
    		...
    		if ((ship.getCurrentCR() <= CRmarker)) { 
    			...
    			float CRRefit = Math.min(ship.getCurrentCR() + CRrestored, CRmax); //Add CRrestored up to the maximum 
    ...
    public void evaluateCircumstances() {
        ...
        if(returning && ship.isFighter() && ship.getWing() != null) {
    	...
    	if (ship.isFighter() && ship.getWing() != null) {
    	    ...
    	    for (ShipAPI carrierAI : CombatUtils.getShipsWithinRange(ship.getLocation(), 8000.0F)) { //Checks for carrier within range.
    	        if (carrierAI.getOwner() != ship.getOwner() 
    	        		|| carrierAI.getVariant() == null 
    	        		|| carrierAI.isPhased()
						|| carrier.getNumFighterBays() == 0
						|| carrierAI.isFighter()) {
    	          continue;
    	        }
    	        if (carrierAI.getOwner() == ship.getOwner()) {
    	    ...
    	    for (ShipAPI fighterAI : CombatUtils.getShipsWithinRange(ship.getLocation(), 4000.0F)) { //Check for fighters nearby to use their landing bay.
        	    if (fighterAI.getOwner() != ship.getOwner() 
        	    		|| !fighterAI.isFighter() 
        	    		|| fighterAI.getWing() == null 
        	    		|| fighterAI.getWing().getSourceShip() == null 
        	    		|| fighterAI.getWing().getSourceShip().isFighter()
        	    		|| fighterAI.getWing().getSourceShip() == ship) {
      	          continue;
      	        }
      	    
     	    	if (fighterAI.getOwner() == ship.getOwner() && fighterAI.getWing().getSourceShip() != ship) {
      	        	if (ship == playerShip) //If it is the player, display the text.
    	    	 ...
    	    	 if (fighterAI != null && fighterAI.getWing().getSourceShip() != null) {
